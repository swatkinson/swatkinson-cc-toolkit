...
`cd` into the caller-provided worktree path; never touch the primary checkout / `main`.
...
4. Re-run `bun run check` (and full `bun run test` if you changed logic). Green — pre-existing unrelated failures are OK; add no new ones.
5. **Git: do NONE.** Leave your changes in the worktree; the caller commits + pushes.

**Genuine-bail only:** a comment that needs a product decision, or that would require editing a hard-rule file (`auth.ts` / `permissions.ts` / env / deploy) → do NOT fix; report it as a handback item.

**Report back:** which comments you addressed (by priority) and how, the scope-deferred P2/P3 you intentionally left (and why), any handback items, and the `check`/`test` results.
