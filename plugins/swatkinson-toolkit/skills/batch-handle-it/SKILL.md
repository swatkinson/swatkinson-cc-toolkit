...
**Re-running is safe.** Each issue gets a deterministic chain of UUIDv5 thread-id slots, so a launch is idempotent by construction: the script resolves the issue's slot against the command read model and skips an issue whose thread is already open, un-archives one that was archived, and steps past one that was deleted. That keys on the **id**, never the title — a thread renamed live from the web UI or the phone is still recognised, so **never delete a running thread just to rename it**. The title passed here is only the starting name.
...
- **A half-opened thread is rolled back.** If the opening command lands and `thread.turn.start` fails, the script undoes it — `thread.delete` for a thread it created, `thread.archive` for one it un-archived — so slot resolution doesn't treat it as already-open on the next run and leave it forever unprompted. The delete costs that slot permanently, which is what the chain is for. If the rollback itself fails, the report names the thread id and the command that didn't take.
...
- **Archiving a thread in the t3 UI does not stop it.** Archiving only sets `archivedAt`; the provider session keeps running. To actually finish one, dispatch `thread.session.stop` (or `thread.delete`).
- **An archived issue is re-opened, not re-created.** A re-run on an archived-but-unfinished issue dispatches `thread.unarchive` + `thread.turn.start` against the slot it already occupies. If that turn fails the thread is re-archived rather than deleted — it predates the run and carries history a delete would destroy.
- **Thread ids are deterministic, and single-use.** Slot 1 for an issue is UUIDv5 over `batch-handle-it:<issue-id>`; later slots append `#2`, `#3`, … up to 50. The chain exists because an id can never be reused: `thread.create` runs `requireThreadAbsent`, which matches against the read model, and a deleted thread stays in that read model with `deletedAt` set. One id per issue would strand the issue permanently the first time someone deleted its thread. Resolution walks the chain and takes the first slot that is free, archived (un-archive it), or live (skip); it steps past deleted ones and fails loudly if all 50 are spent. The namespace is distinct from any sibling batch skill's, so two skills launching the same issue id can't collide onto one thread.
- **Slot resolution reads `/api/orchestration/snapshot`, not `/shell`.** The shell snapshot omits archived threads entirely, so their ids look free while the decider still holds them — resolving against it would try to re-create an id the server would reject. The command read model carries archived and deleted rows, which is what the chain has to see. It's the same call that resolves projects, so this costs no extra request.
