...
2. **5/5 (no P0/P1)?** → run **one final FULL-diff review** (`claudecodile-reviewer`, full pass) to catch cross-cutting regressions the incremental view hides. Still 5/5 → **return success** — but first confirm no inline thread for a fixed finding is still open (the reviewer resolves these each round; if any linger, resolve them before returning). Else keep looping. Never declare 5/5 on an incremental round.
