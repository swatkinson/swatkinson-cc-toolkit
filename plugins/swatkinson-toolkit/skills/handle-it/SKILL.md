---
name: handle-it
description: End-to-end orchestrator that takes a tracked issue (or a freeform feature/project) and drives it through planning → implementation → draft PR → an in-house review⇄fix loop (rated to 5/5) → conflict-free CI + preview → an auto-tester → your manual review (PR stays a draft) → ready-for-review on your approval, with a live status table and tracker blocking awareness. On un-draft, PRs scoring Quality 5/5 ∧ Spec 5/5 ∧ Risk ≥ 4 are auto-approved by the Swat Reviewer bot (fast-pathed, no re-review); riskier ones route to a senior. Stops after un-drafting — merge is manual; on your word that it merged, it deletes the branch + worktree. Use when the user invokes /handle-it, or asks to take an issue/feature all the way to a review-ready PR.
---
...
**Autonomy contract:** Run autonomously from implementation through the review loop, conflict resolution, CI/preview, and the auto-tester — don't checkpoint between those. The PR is created **without a confirm gate** — Phase 5 builds the title/body and opens the draft straight away. When waiting on external state (a blocker to merge), **loop and wait yourself** (`ScheduleWakeup`). The human gates are exactly: (a) `/grill-with-docs` during planning, (b) a **setup-secret ask** when the config flags one as needed for a migration/DB change (e.g. a DB connection string, or confirming a copied `.env`) — only for that change type; a non-migration change just quick-confirms and proceeds, (c) the **manual-review gate** — the PR stays a **draft** through review/CI/preview/test-and-tick; you hand it off for manual testing and WAIT; on the user's "looks good" you un-draft (mark ready for review) — **then stop**, (d) a **bail** when you genuinely cannot proceed (`PushNotification`, update the table, stop).
...
4. **Detect where to resume.** `/handle-it` can be dropped on an issue at **any** stage — don't assume Phase 1. Derive the furthest-completed phase and jump straight there:
   ...
   - **Else derive from ground truth** (authoritative — the tracker's status block is only a hint, and can be stale): worktree present (config → Commands → worktree list)? PR for the branch (`gh pr view`)? draft or ready? is there a `## 🪰 Swat Reviewer Rating` comment and what are its Quality/Spec scores? CI state, `mergeable`, `reviewDecision`, `state`? Map these to the entry phase (full table in REFERENCE → Resume detection).
...
2. **Create the draft (no confirm gate):** `gh pr create --draft --title <title> --body <body> --base <base>` from the worktree (pass the body via `--body-file` or a HEREDOC-literal `--body "$(cat <<'EOF' … EOF )"` — never `--body "@path"`). Default `--base` = the repo's base branch; on a **stacked** run use `--base <blocker-branch>` (REFERENCE → Stacked PRs). Phase 4 already ran the verify gate, so don't re-verify here.
3. **Capture the PR number**, comment the PR URL on the issue (per config → tracker), and keep status **In Progress**.
...
3. `gh pr ready <N>` — **un-draft**. This fires `ready_for_review`; on a **bot-enabled (CI-mode) repo** Swat Reviewer re-runs on the now-non-draft PR and, if it already scored **Quality 5/5 ∧ Spec 5/5 ∧ Risk and Complexity ≥ 4** on the current head, **fast-paths straight to a formal Approve** (no full re-review — the un-draft shipped no new code, so the prior rating still stands). Risk ≤ 3, or any gating facet < 5/5 → no approval. The App is a non-author identity, so the approval is valid and counts toward branch protection (`/handle-it` running as the author cannot self-approve).
4. **Assign yourself:** `gh pr edit <N> --add-assignee @me` — set the current user as the PR Assignee.
5. Move the issue → **In Review** and comment that it's review-ready (per config → tracker; skip in trackerless mode).
6. **Tell the user, keyed to the rating's Risk score:**
   - **Risk ≥ 4:** "#<N> is ready — Q5 Sp5 · R<n>; auto-approved by Swat Reviewer on un-draft (on a bot-enabled repo), clear to merge."
   - **Risk ≤ 3:** "#<N> is ready, but Risk is <n>/5 — request review from a senior before merging."
   (In local mode / a repo without the bot, there's no App to approve — say it's ready and, for Risk ≤ 3, route to a senior.)
   Either way, add: **"Let me know once it's merged and I'll delete the git branch + worktree for you."**
