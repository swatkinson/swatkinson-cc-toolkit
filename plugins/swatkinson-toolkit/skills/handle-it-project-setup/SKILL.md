...
4c. **Code review (CI?).** This skill **installs** the Swat Reviewer bot workflow in Phase 2 (step 4) unless the user opts out, so default `Swat Reviewer runs in CI` to **true** when you install it (and leave it `true` if `.github/workflows/*` already has a swat-review action). Set it **false** only if the user opts out of the CI bot — then the review runs locally inside `/handle-it` instead.
...
4. **Install the Swat Reviewer bot workflow** (the CI reviewer). Unless the user opts out, copy the bundled caller verbatim into the repo:
   - Source: `${CLAUDE_PLUGIN_ROOT}/skills/handle-it-project-setup/caller-workflow.yml` → Target: `.github/workflows/swat-review.yml` (create `.github/workflows/` if absent).
   - **If the target already exists, do NOT clobber it** — diff it against the bundled copy, mention any drift, and leave the existing file unless the user asks to overwrite.
   - It runs the bot on every PR and, when a branded App identity is configured, posts as that App (enabling thread resolution + risk-based auto-approval). Prerequisites are org/repo-level: a `CLAUDE_CODE_OAUTH_TOKEN` secret, and optionally the `SWAT_REVIEWER_APP_PRIVATE_KEY` secret + `SWAT_REVIEWER_APP_ID` variable for the App identity (see the toolkit repo's `docs/github-bot/README.md`). If the org already has these set, committing this caller is all a repo needs — don't create Apps/secrets yourself; surface what's missing instead.
   - **Opt-out:** if the user doesn't want CI review on this repo, skip this step and set `Swat Reviewer runs in CI: false` in `config.md` (the review then runs locally inside `/handle-it`).
5. **Tell the user** it's ready and that `/handle-it` and `/swat-review` will now use it — point out the `rules/` files as the knobs for PR/commit/review formatting, the installed `.github/workflows/swat-review.yml`, and note the engine will self-correct the config/rules as it learns. If `.gitignore` excludes `.claude/`, mention they may want to commit `.claude/handle-it/` (and the new workflow) so teammates share them.
